...
def printEverything():
    ...
    for point in points:
        ...
        if(xy[2] > 0):
            ...
            dispY = int(round(xy[1] / 2.0)) #I just happen to be using a terminal with a 2:1 aspect ratio for text characters.
...
#allows player to input points. just for testing really
def makeAPoint():
    print("X:")
    newX = float(input())
    print("Y:")
    ...
    print("Symbol:")
    newSymbol = input()[0]
    points.append([newX,newY,newZ,newSymbol])
...
while True:
    printEverything()
    print("(",playerPos[0],",",playerPos[1],",",playerPos[2],")")
    takePlayerInput()