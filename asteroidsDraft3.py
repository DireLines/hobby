    ...
    def takeGravityIntoAccount(self, otherAsteroids):
        ...
        for asteroid in otherAsteroids:
            if("emitsGravity" in asteroid.properties and asteroid != self):
                ...
                accelVector = getDirectionVector((self.x,self.y) , (asteroid.x,asteroid.y) , m / r**2)
...
class History: #positions of stuff over time
    def __init__ (self, data=[], steps=600, frame=0):
        self.data = data
        self.steps = steps
        self.frame = frame
...
def seedBoard(board):
    baseVelocity = 0.5
    #Rock at the center
    board.append(Asteroid(symbol='R', mass=14, properties=["emitsGravity"]))
    board.append(Asteroid(symbol='G',x=5,y=5,vel_x = random.uniform(-2.3*baseVelocity,2.3*baseVelocity), vel_y = random.uniform(-2.3*baseVelocity,2.3*baseVelocity), properties = ['affectedByGravity']))
